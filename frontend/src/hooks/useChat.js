    ...
    try {
      ...
      if (currentConversation?.id === convId) {
        setCurrentConversation(prev => {
          ...
          try {
            localStorage.setItem('ailrac_current_conv', JSON.stringify(updated))
          } catch (e) {}
          return updated
        })
      }
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      ...
      if (data.is_new_conversation || !currentConversation) {
        ...
        try {
          localStorage.setItem('ailrac_current_conv', JSON.stringify(data.conversation))
        } catch (e) {}
      }

      // Replace optimistic message with real confirmed messages
